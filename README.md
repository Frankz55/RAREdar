...
- Merge this branch into `main` when you're done.

Comments/questions for `RAREdar.py`:
- Is `ReverseList` the reverse or the reverse complement of `MotifList`?  Add comments that carefully explains this.
- Consider renaming `Gene_Sequence_list.fa` with the species name, in case we ever want to run it on another file.
